...
def preprocess(img_dict, lr_dim, upscale_factor=UP_SAMPLE):
    ...
    img = img_dict['image']
    img_dict['image'] = tf.image.resize(img, lr_dim), tf.image.resize(img, hr_dim)
...
@tf.function
def gen_train_step(high_res_imgs, low_res_imgs, gan_output_res):
    """
    high res image
    low res image
    ...
    with tf.GradientTape() as tape:
        ...
        predictions = ProGAN.Generator(high_res_imgs, training=True)
        ...
        with image_summary_writer.as_default():
            ...
            tf.summary.image("high res", high_res_imgs, step=0)
            tf.summary.image("low res", low_res_imgs, step=0)
...
def train_step(epoch, save_c, gan_output_res):
    ...
    if not epoch % 2:
        ...
        for i, batch in enumerate(train_ds):
            ...
            hr, lr = batch['image']
            gen_train_step(hr, lr, gan_output_res)
            # update alpha
            update_fadein(i, train_ds_size)
...
train_ds, test_ds, train_ds_size = prepare_and_upscale(lr_dim=(8, 8))
